...
def is_member_of_set(setB,member) :
...
def my_set_store():
	...
	while(True):
		...
		if choice   ==1:
					...
					result=is_member_of_set(setB,member)
					print(f"member is:{result}")                          
		elif choice ==6:
					result=set_display(setA)
					print(f"Elements are :{result}")
		elif choice ==7:
					result=set_display(setB)
					print(f"Elements are :{result}")
		elif choice ==8:
					member= input("Please enter a number:")
					set_add_element(setA,member)
		elif choice ==9:
					members= set(input("Please enter comma seperated elements for the set A").split(","))
					set_add_elements(setA,members)
		elif choice ==10:
					member=input("Enter a element:")
					set_remove_element(setA,member)
		elif choice ==11:
					break
		else:
